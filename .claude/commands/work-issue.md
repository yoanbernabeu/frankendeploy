...
2. Explore the codebase using FrankenDeploy search to understand related code
...
This creates the binary at `./bin/FrankenDeploy`.
...
./bin/FrankenDeploy <command-to-test>

# Examples:
# - New CLI command: ./bin/FrankenDeploy new-command --flag
# - Search fix: ./bin/FrankenDeploy search "test query"
# - Trace feature: ./bin/FrankenDeploy trace callers "FunctionName"
# - Config change: ./bin/FrankenDeploy init && cat .FrankenDeploy/config.yaml
```

**You MUST:**

- Build the CLI binary with `make build`
- Execute the CLI binary (`./bin/FrankenDeploy`) to test your changes
...
- **Use FrankenDeploy** for code exploration (as per project guidelines)
