...
Deploying from GitHub Actions, GitLab CI or any runner takes the CLI, a deploy key and a server entry: complete recipes, the `--yes` semantics and the environment variables are in the [CI/CD guide](/frankendeploy/guides/ci-cd/).
